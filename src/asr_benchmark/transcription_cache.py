...
class TranscriptionCache:
    ...
    def set_model_metadata(
        ...
        """保存模型报告信息，同时保留该模型已经完成的样本。"""

        existing = self._models.get(model_name)
        transcriptions = dict(existing.transcriptions) if existing else {}
...
def _parse_cached_model(
    ...
    return CachedModel(report_name, device, load_latency_seconds, transcriptions)
